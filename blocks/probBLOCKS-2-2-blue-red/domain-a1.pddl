(define (domain blocks-2-2-blue-red-a1)
	(:requirements :factored-privacy :probabilistic-effects :typing)
	(:types
		agent block - object
		blueAgent redAgent - agent
		blue red - block
	)
	(:predicates
		(on ?x - block ?y - block)
	...
	(:action action-b-pick-up-a-stack
	...
	(:action action-d-put-down-a-stack
		:parameters (?a - agent ?x - block ?y - block)
		:precondition
	...
	(:action action-h-unstack-a-stack
		:parameters (?a - agent ?x - block ?y - block ?z - block)
		:precondition (and
			(on ?x ?z)
			(on ?y ?x)
