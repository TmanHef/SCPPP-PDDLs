(define (domain blocks-2-2-blue-red-a2)
	...
	(:action action-j-pick-up-a-stack
	...
	(:action action-l-put-down-a-stack
		:parameters (?a - agent ?x - block ?y - block)
		:precondition
	...
	(:action action-p-unstack-a-stack
		:parameters (?a - agent ?x - block ?y - block ?z - block)
		:precondition (and
			(on ?x ?z)
			(on ?y ?x)
